...
export async function GET(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    const completedMap = new Map<
      string,
      { tallerName: string; branch: string; completedAt: string | null }[]
    >();
    ...
    const result = inscripciones
      .map((insc) => ({
        ...
        profile: {
          ...
          rut: insc.profile.rut,
          blood_type: insc.profile.blood_type,
          allergies: insc.profile.allergies,
          medications: insc.profile.medications,
          medical_conditions: insc.profile.medical_conditions,
          emergency_contact: insc.profile.emergency_contact,
          emergency_phone: insc.profile.emergency_phone,
        },
    ...
    console.error('[coordinador inscripciones GET]:', error);
...
export async function PATCH(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    console.error('[coordinador inscripciones PATCH]:', error);
    return NextResponse.json({ error: 'Error interno del servidor' }, { status: 500 });
  }
}
